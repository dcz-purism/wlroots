...
static struct roots_text_input *relay_get_active_text_input(
...
static void handle_im_commit(struct wl_listener *listener, void *data) {
	...
	struct roots_text_input *text_input = relay_get_active_text_input(relay);
	if (!text_input) {
		return;
	}
...
static void relay_activate_text_input(struct roots_input_method_relay *relay,
		struct roots_text_input *text_input) {
	assert(relay == text_input->relay);
	...
	if (!input_method) {
		wlr_log(WLR_DEBUG,
			"No input method present on this seat, not activating");
		return;
	}
	wlr_input_method_v2_send_activate(input_method);
}

static void relay_deactivate_text_input(struct roots_input_method_relay *relay,
		struct roots_text_input *text_input) {
	assert(relay == text_input->relay);
	...
	if (!input_method) {
		wlr_log(WLR_DEBUG,
			"No input method present on this seat, not deactivating");
		return;
	}
	wlr_input_method_v2_send_deactivate(input_method);
}

static void relay_update_text_input_states(
		struct roots_input_method_relay *relay,
		struct roots_text_input *changed_text_input) {
	// TODO: simplify by switching active state of the text input inside this function.
	// map(lambda t: t.active = false, relay.text_inputs)
	// text_input?.active = text_input.enabled
	assert(relay == changed_text_input->relay);

	struct roots_text_input *active_text_input = NULL;
	struct roots_text_input *cur_text_input;
	wl_list_for_each(cur_text_input, &relay->text_inputs, link) {
	...
	if (changed_text_input->enabled) {
		if (changed_text_input != active_text_input) {
			if (active_text_input) {
				relay_deactivate_text_input(relay, active_text_input);
			}
			relay_activate_text_input(relay, changed_text_input);
		}
	} else if (changed_text_input->active) {
		relay_deactivate_text_input(relay, changed_text_input);
	}
}
...
static void handle_text_input_enable(struct wl_listener *listener, void *data) {
	...
	wlr_log(WLR_DEBUG, "Text input enable received");
	text_input->enabled = true;
	relay_update_text_input_states(relay, text_input);
...
static void text_input_disable(struct roots_input_method_relay *relay,
		struct roots_text_input *text_input) {
...
static void handle_text_input_disable(struct wl_listener *listener,
	...
	struct roots_text_input *text_input = text_input_to_roots(relay,
		(struct wlr_text_input_v3*)data);
	if (!text_input->enabled) {
		wlr_log(WLR_INFO, "Inactive text input tried to disable itself");
		return;
	}
	text_input_disable(relay, text_input);
}
